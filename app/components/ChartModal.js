...
import { calculateAverageIntake, calculateDayAverageLoss, calculateDayMetabolicRate, sortEntries } from '../constants';
...
export default function ChartModal({ user, updateUser }) {
    ...
    if (daysToGraph && daysToAverage) {
        ...
        for (var i = 1; i < entries.length; i++) {
            ...
            const curBasalRate = parseFloat(calculateDayMetabolicRate(user, curEntry));

            const curWeight = parseFloat(calculateDayAverageLoss(user, curEntry, daysToAverage));
            const nextWeight = parseFloat(calculateDayAverageLoss(user, nextEntry, daysToAverage));

            const curCalories = parseFloat(calculateAverageIntake(user, curEntry, daysToAverage));
            const nextCalories = parseFloat(calculateAverageIntake(user, nextEntry, daysToAverage));
    ...
    return (
        <>
            <Button sx={{ marginTop: 2, marginLeft: 2 }} color="primary" variant="contained" size="small" onClick={() => setShow(!show)}>
